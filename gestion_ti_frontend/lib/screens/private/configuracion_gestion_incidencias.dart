...
class ConfigGestionInc extends StatefulWidget {
  const ConfigGestionInc({super.key});

  @override
  State<ConfigGestionInc> createState() => _ConfigGestionInc();
}

class _ConfigGestionInc extends State<ConfigGestionInc> {
  final SupabaseClient supabase = Supabase.instance.client;
  bool _isLoading = false;

  final TextEditingController _searchController = TextEditingController();
  List<dynamic> _incidenciaTipos = [];
  ...
  _getData() async{
    try{
      ...
      _incidenciaTipos = data.where((element) => element['elemento'] == 'incidencia_tipo').toList()[0]['valores'];
    } catch(e){
  ...
  _save() async{
    try {
      ...
      final response = await supabase
          .from('configuracion_general')
          .update({'valores': _incidenciaTipos,})
          .eq('modulo', 'gestion_incidencias')
          .eq('elemento', 'incidencia_tipo');
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      children: [
                        Row(
                          children: [
                            SimpleTable(dataList: _incidenciaTipos, title: 'Tipos de incidencias',),
                          ],
                        ),
                        SizedBox(height: 500,)
                      ],
